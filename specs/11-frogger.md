...
## Notas de implementación (SPEC 11 en Arcade Vault)

La spec la redactó el agente `game-jam` contra convenciones que no coinciden con este
repo. Se implementó adaptándola a los patrones reales de la plataforma (aprobado antes de
empezar):

- **`game-id` = `ranaria`**, no `frogger`. El catálogo ya tenía la ficha simulada `ranaria`
  (migración 06) reservada para este port. En vez de un `INSERT` nuevo, la migración
  `supabase/migrations/10-juego-ranaria.sql` solo activa `has_leaderboard = true` en esa
  fila (mismo patrón que la 09 de Snake). Carpeta interna: `app/components/games/frogger/`.
- **Motor agnóstico + envoltorio**, no `components/games/FroggerGame.tsx` monolítico:
  `frogger/engine.ts` (clase `FroggerGame`, sin React/Next) + `frogger/frogger-player.tsx`
  (`"use client"`), registrado como `ranaria` en `app/components/games/registry.ts`.
- **Sin play-page propia**: `/juego/[id]/jugar` enruta por `REAL_GAME_PLAYERS`. No existe
  `app/games/frogger/play/page.tsx`.
- **Guardado de score** vía `submitScore()` (`app/lib/scores-actions.ts`), que valida en
  servidor y revalida `/salon` + `/juego/ranaria`. No hay insert directo a Supabase ni
  `localStorage av_player_name`; las iniciales se piden en el overlay React del envoltorio.
- **HUD**: solo el HUD interno del canvas (score, nivel, vidas, barra de tiempo). No hay
  "doble HUD" ni props `onScoreChange/onLivesChange/onLevelChange` — ninguno de los 4
  juegos reales previos los tiene. El motor solo expone `onGameOver({ score, level })`.
- **Teclado** gestionado dentro del motor (`start`/`stop`), como Snake; el envoltorio solo
  añade pausa (`Esc`/`P`) y cierre de overlay.
- **Skins**: `FROGGER_SKINS` (clásico/neón/retro) andamiadas en `engine.ts`; el afinado
  visual de neón/retro lo hace el agente `skin-designer`.
- **Canvas 640 × 560** (16 × 14 celdas de 40 px), según el plan y las Decisions; la
  mención suelta de "480 × 640" en el Scope se ignora.
- **Wrap de entidades**: reintroducción periódica por `span` (periodo del carril) en vez de
  teletransporte a `col = -width` / `col = COLS`, para que el espaciado entre vehículos y
  troncos no se deforme con las vueltas.
