...
- `game-performance` (`.claude/agents/game-performance.md`) — mide y mejora el rendimiento en runtime de **un juego con motor real por invocación**: perfila el bucle de canvas con Playwright (frame time p50/p95, allocations por frame, coste de render), aplica optimizaciones **invisibles** (cache de capas estáticas en un canvas offscreen, cero allocations por frame, menos `save/restore`, gradientes y `ctx.font` fuera del loop) y vuelve a medir para probar la ganancia. Render **idéntico pixel a pixel** y jugabilidad intacta. Mantiene el registro en `references/game-performance.md`. Toca solo `engine.ts` y `<DIR>-player.tsx` de ese juego; no toca mecánica, puntuación, `shadowBlur`/efectos, `registry.ts`, `app/globals.css`, `scores` ni migraciones.
