# Skins por juego — Arcade Vault

Registro de qué juegos con motor real tienen implementado el sistema de skins.
Lo mantiene el agente `skin-designer` (`.claude/agents/skin-designer.md`), que trabaja
**un juego a la vez** y solo sobre el juego que se le indique.

## Contrato

Cada juego que "cumple" tiene, en `app/components/games/<DIR>/engine.ts`:

- `interface <Nombre>Palette` — un rol de color por uso del canvas.
- `const <NOMBRE>_SKINS: Record<SkinName, <Nombre>Palette>` con las claves exactas
  `clasico` · `neon` · `retro`.
- `skin: SkinName = "clasico"` + `setSkin(name)` que aplica al vuelo.

Y en `app/components/games/<DIR>/<DIR>-player.tsx`:

- `<SkinPicker gameId value onChange>` fuera del `<canvas>` y del overlay.
- Preferencia cargada con `loadSkin(GAME_ID)` y guardada con `saveSkin(GAME_ID, skin)`
  (`localStorage`, clave `arcade-vault:skin:<gameId>`).

Skins:

- **`clasico`** (por defecto) — la paleta original del juego, sin cambios.
- **`neon`** — paleta casa saturada cian/magenta/amarillo/verde sobre negro, glow CRT.
- **`retro`** — fósforo monocromo cálido (ámbar/verde), bajo contraste, sin glow.

## Infra compartida

| Archivo                                   | Estado |
| ----------------------------------------- | ------ |
| `app/components/games/skins.ts`           | Hecho  |
| `app/components/games/skin-picker.tsx`    | Hecho  |
| `app/globals.css` · bloque `.skin-picker` | Hecho  |

La primera invocación del agente que implemente skins crea esta infra.

## Estado por juego

Fuente de la lista: `REAL_GAME_PLAYERS` en `app/components/games/registry.ts`.

| ID              | Motor (`<DIR>`) | Skins                     | Fecha      | Notas                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------- | --------------- | ------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rocas`         | `asteroids`     | ✅ clasico + neon + retro | 2026-08-29 | Paleta `AsteroidsPalette` (11 roles): bg, ship, thruster, bullet, asteroid, particle (triple `r,g,b` para rgba con alfa), hud, hudDim, accent, overlayTitle, glow (shadowBlur). Las rocas, la nave, las balas y las vidas son wireframe; el power-up 3x usa `accent`.                                                                                                                                                                                               |
| `caida`         | `tetris`        | ❌ ninguna                | —          |                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `bloque-buster` | `arkanoid`      | ✅ clasico + neon + retro | 2026-08-29 | Paleta `ArkanoidPalette` (12 roles): bg, floorLine, ball, paddle, paddleHighlight, blockHighlight, hudScore, hudLevel, hudLives, pauseText, pauseSub, glow (shadowBlur px) + `blocks` (Record de 7 claves del referente: red/yellow/cyan/magenta/hotpink/green/gray). Los bloques guardan `colorKey` y el tono se resuelve en draw(), así `setSkin()` conmuta al vuelo. neon: glow 14, bola amarilla; retro: ámbar monocromo, filas separadas por brillo, sin glow. |
| `serpentina`    | `snake`         | ✅ clasico + neon + retro | 2026-08-29 | Paleta `SnakePalette` (12 roles): bg, grid, hudRule, snakeHead, snakeBody, fruit, hudScore, hudLabel, hudAccent, overlayTitle, overlayDim, glow (shadowBlur de serpiente y fruta). La fruta usa el spritesheet PNG `public/games/serpentina/fruits.png` (la paleta no lo toca); `fruit` solo colorea el rombo procedural de respaldo previo a la carga del sprite.                                                                                                  |

Leyenda de la columna **Skins**: `❌ ninguna` · `✅ clasico + neon + retro` · `⚠️ parcial`.
